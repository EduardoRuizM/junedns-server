...
const fromIPv6 = (address) => {
  ...
  // CAVEAT edge case for :: and IPs starting or ending by ::
  if(digits[0] === '')
    digits.shift();
  ...
  // CAVEAT we have to take into account the extra space used by the empty string
...
Packet.Resource.NAPTR = {
  decode: function(reader, length) {
    this.order = reader.read(16);
    this.preference = reader.read(16);
    this.flags = '';
    const lflags = reader.read(8);
    for(let i = 0; i < lflags; i++) this.flags+= String.fromCharCode(reader.read(8));
    this.service = '';
    const lservice = reader.read(8);
    for(let i = 0; i < lservice; i++) this.service+= String.fromCharCode(reader.read(8));
    this.regexp = '';
    const lregexp = reader.read(8);
    for(let i = 0; i < lregexp; i++) this.regexp+= String.fromCharCode(reader.read(8));
    this.replacement = Packet.Name.decode(reader);
    return this;
  },
  encode: function(record, writer) {
    writer = writer || new Packet.Writer();
    const { length } = Packet.Name.encode(record.target);
    const lflags = record.flags.length;
    const lservice = record.service.length;
    const lregexp = record.regexp.length;
    writer.write(length + 7 + lflags + lservice + lregexp, 16);
    writer.write(record.order, 16);
    writer.write(record.preference, 16);
    writer.write(lflags, 8);
    for(let i = 0; i < lflags; i++) writer.write(record.flags.charAt(i), 8);
    writer.write(lflags, 8);
    for(let i = 0; i < lservice; i++) writer.write(record.service.charAt(i), 8);
    writer.write(lregexp, 8);
    for(let i = 0; i < lregexp; i++) writer.write(record.regexp.charAt(i), 8);
    Packet.Name.encode(record.target, writer);
    return writer.toBuffer();
  },
};

Packet.Resource.TLSA = {
  decode: function(reader, length) {
    this.usage = reader.read(8);
    this.selector = reader.read(8);
    this.matchingtype = reader.read(8);
    this.buff = Packet.Name.decode(reader);
    return this;
  },
  encode: function(record, writer) {
    writer = writer || new Packet.Writer();
    const { length } = Packet.Name.encode(record.buff);
    writer.write(length + 3, 16);
    writer.write(record.usage, 8);
    writer.write(record.selector, 8);
    writer.write(record.matchingtype, 8);
    Packet.Name.encode(record.buff, writer);
    return writer.toBuffer();
  }
};
